...
project_table = "create table project (name text unique, recovery integer, datetime text);"
#bug_table = "create table bug (name text, priority text, status text, description text);" 
bug_table = "create table bug (name text, description text, priority text, status text, projectid integer, foreign key(projectid) references project(rowid));" 
